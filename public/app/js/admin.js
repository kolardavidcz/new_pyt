...
export function updateAdminUIElements() {
  ...
  if (actBtn) {
    actBtn.style.display = admin ? "flex" : "none";
  }
      ...
      <div class="modal-body" style="padding:16px 20px;">
        ...
        <div class="admin-tab-content ${activeTab === "users" ? "active" : ""}" id="tab-users">
          ...
          <div style="background:var(--bg-elevated, #252526); border:1px solid var(--border-subtle, #333); border-radius:8px; padding:14px; display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
            ...
            <button type="button" class="btn primary sm" id="btnAdmCreateAdmin">Přidat Admina 🛡️</button>
        ...
        <div class="admin-tab-content ${activeTab === "diagnostics" ? "active" : ""}" id="tab-diagnostics">
          ...
          <div style="background:var(--bg-elevated, #252526); border:1px solid var(--border-subtle, #333); border-radius:8px; padding:16px; display:flex; flex-direction:column; gap:12px;">
            ...
            <div style="display:flex; gap:10px; flex-wrap:wrap;">
              <button type="button" class="btn secondary sm" id="btnAdmForceSync">Vynutit Cloud Sync ☁️</button>
              <button type="button" class="btn secondary sm" id="btnAdmClearErrors">Vymazat Logy Chyb 🗑️</button>
            </div>
      ...
      return `
        <div class="admin-item-card" data-id="${item.id}">
          ...
          <div class="admin-item-actions">
            ...
            <button type="button" class="admin-btn-sm danger btn-act-delete" data-id="${item.id}">Smazat 🗑️</button>
          </div>
        </div>
      `;
    }).join("");
      ...
      return `
        <div class="admin-item-card">
          <div class="admin-item-head">
            ...
            <span class="admin-status-pill ${isAdmin ? "resolved" : "dismissed"}">${isAdmin ? "ADMIN 🛡️" : "STUDENT 🎓"}</span>
          </div>
          ...
          <div class="admin-item-actions" style="align-items:center;">
            ...
            ${u !== "kolard" ? (
              isAdmin
                ...
                : `<button type="button" class="admin-btn-sm btn-act-grant-admin" data-user="${escapeHtml(u)}">Povýšit na Admina 🛡️</button>`
...
function renderRelevanceTab() {
  ...
  function starsStr(rel) {
    const val = Math.max(1, Math.min(10, rel || 1));
  ...
  function updateList() {
      ...
      return `
        <div class="admin-rel-row">
          ...
          <div style="display:flex; align-items:center; gap:12px;">
            <span style="color:#eab308; font-size:14px; letter-spacing:1px;" class="rel-stars-preview">${starsStr(rel)}</span>
    ...
    container.querySelectorAll(".rel-num-input").forEach((inp) => {
      inp.addEventListener("input", () => {
        ...
        const starsEl = row?.querySelector(".rel-stars-preview");
        if (starsEl) starsEl.textContent = starsStr(val);
      });
    });
...
function renderDiagnosticsTab() {
  ...
  if (forceSyncBtn) {
    forceSyncBtn.onclick = async () => {
      ...
      forceSyncBtn.textContent = "Vynutit Cloud Sync ☁️";
